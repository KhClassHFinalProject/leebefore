		...
		<div class="col-md-9">
			...
			<div class="row">
				<div class="col-md-10">
					...
					<div class="row">
					<form action="questList.ju" id="frm">
							<div class="col-md-2">
								<select class="form-control" name="type">
									<option value="qu_subject">전체</option>
									<option value="mem_idx">작성자</option>
									<option value="qu_subject">제목</option>
								</select>
							</div>
								...
								<ul class="pagination pagination-sm">
									<c:choose>
										<c:when test="${page==1}">
											...
											<c:otherwise>
												<li><a href="questList.ju?page=${page-1}"><span class="glyphicon glyphicon-chevron-left"></span></a></li>
											</c:otherwise>
									</c:choose>
									<c:forEach begin="${startPage}" end="${endPage}" var="i">
										<c:choose>
											<c:when test="${i eq page}">
												<li class="active"><a href="questList.ju?page=${i}">${i}</a></li>
											</c:when>
											<c:otherwise>
												<li><a href="questList.ju?page=${i}">${i}</a></li>
											</c:otherwise>
										</c:choose>
									</c:forEach>
									<c:choose>
										<c:when test="${page==endPage}">
											...
											<c:otherwise>
												<li><a href="questList.ju?page=${page+1}"><span class="glyphicon glyphicon-chevron-right"></span></a>
											</c:otherwise>
									</c:choose>
								</ul>
							</div>
